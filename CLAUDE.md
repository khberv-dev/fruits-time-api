...
Two events fire. `OrderService.notifyDeliveryOrderCreated`, posted when a `DELIVERY` order is created (POS id, branch, customer name/phone, business-local timestamp), and `SubscriptionService.postRequestToGroup`, posted when a subscription request is raised — that one carries an **inline button** whose `callback_data` is `subreq_accept:<requestId>` (kept short; Telegram caps `callback_data` at 64 bytes and a UUID is already 36).

Pressing it reaches `POST /api/subscription/telegram/callback` (`SubscriptionTelegramController`), which is `@IsPublic()` to the JWT guard but gated by `TelegramWebhookGuard` on `TELEGRAM_WEBHOOK_SECRET`, matched against the `x-telegram-bot-api-secret-token` header Telegram echoes from `setWebhook`'s `secret_token`. `handleTelegramCallback` flips the request to `accepted`, answers the callback (Telegram spins the button until answered), and rewrites the card **without its button** so it can't be actioned twice. It answers on every path — unknown action, missing request, already accepted — and tolerates a callback with no `message` by skipping only the edit. **The webhook must be registered manually** (`setWebhook`), and since Telegram allows one webhook per bot, pointing this bot at the API takes over any webhook it already had. It's called **after** the transaction commits and is deliberately not awaited, so Telegram being slow or down neither rolls the order back nor delays the response. Message text lives in `OrderService`, not `TelegramService`, to keep order wording in the order module.
