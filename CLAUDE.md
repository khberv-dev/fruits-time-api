...
**Expiry** runs from redemption, not from a fixed calendar window. `Subscription.durationDays` (null = never expires) is **snapshotted onto `SubscriptionCode.expiresAt` when the code is redeemed** — deliberately not derived on read, so editing `durationDays` later only affects codes redeemed afterwards and can never retroactively shorten, or instantly lapse, an entitlement someone already holds. `getActiveSubscriptions` drops expired codes, which removes them from `GET /subscription/me` *and* stops them discounting orders (`getEntitlement` feeds `computeDiscount`). Holding two codes for the same subscription keeps the most generous expiry, so re-redeeming extends. Rows predating the column have `expiresAt` null and therefore never expire. `describeCode` reports a lapsed code as `expired`, distinct from `inactive` (admin switched the subscription off).
